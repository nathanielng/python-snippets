...
#   streamlit run streamlit_bedrock3.py --server.port 8053 --server.runOnSave true
...
def tab_chat(generator):
    ...
    with col1:
        ...
        with st.form("Feedback"):
            if os.path.isfile("txt/participant_feedback.txt"):
                with open("txt/participant_feedback.txt", "r") as f:
                    participant_feedback = f.read()
            else:
                participant_feedback = ''

            context = st.text_area(label="Feedback Form", value=participant_feedback, height=50)
            prompt_template = st.text_area(
                label="Prompt Template",
                value="The context is: {context}\nBased on the above context, answer the following question: Provide some ideas on how this feedback from a workshop can be improved, in the context of the Rolfe reflective model. Each suggestion should be enclosed in <suggestion> tags.")

            submit_suggestions = st.form_submit_button("Suggest Improvements")
            if submit_suggestions:
                final_prompt = prompt_template.format(
                    context=participant_feedback
                )
                ...
                with st.chat_message("assistant"):
                    ...
                    if len(st.session_state.suggestions) == 0:
                        st.session_state.suggestions = find_suggestions(response)
                    else:
                        new_suggestions = find_suggestions(response)
                        st.session_state.suggestions = st.session_state.suggestions + new_suggestions

            evaluate_based_on_suggestions = st.form_submit_button("Evaluate based on suggestions")
            if evaluate_based_on_suggestions:
...
def ask_multimodal(prompt, image_file, generator):
...
def tab_multimodal(generator):
    ...
    with col2:
        img_glob = st.text_input("Images", value="images/ransack*.png")
    ...
    with col1:
        st.markdown("**Example prompts**\n- What is the sentiment of the author?\n- Suggest some next steps based on the article")
        image_to_query = st.selectbox("Choose an image", options=[i+1 for i in range(len(img_files))], index=0)
        for message in st.session_state.messages3:
            with st.chat_message(message["role"]):
                st.markdown(message["content"])

        if prompt := st.chat_input("Ask a question about the slide", key="Chat with images"):
            ask_multimodal(prompt, img_files[image_to_query-1], generator)
...
def main():
    ...
    tab1, tab3, tab2 = st.tabs(["Chat", "Multimodal", "Query a File"])
    with tab1:
        tab_chat(generator)
    with tab2:
        # tab_rag(generator)
